...
namespace Nutrition.UnitTests.Domain.Events
{
    [Trait("Category", "Unit")]
    [Trait("Layer", "Domain")]
    public class MealFoodAddedEventTests
    {
        [Fact]
        public void Constructor_ValidParameters_ShouldCreateEvent()
        {
            // Arrange
            var diaryId = 1;
            var totalCalories = 350.5m;

            // Act
            var domainEvent = new MealFoodAddedEvent(diaryId, totalCalories);
        ...
        {
            // Arrange & Act
            var event1 = new MealFoodAddedEvent(1, 100m);
            var event2 = new MealFoodAddedEvent(1, 100m);
